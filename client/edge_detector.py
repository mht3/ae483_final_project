...
def draw_lines(image):
    ...
    Variant of DFS path finding algorithm.
    '''
    h, w = image.shape
...
def final_postprocess(image, option=0):
    '''
    Final postprocessing technique.
    Option 0 (default): Dilate the image to remove disconnects and skeletonize (thin lines). Preferred method for DFS algorithm.
    Option 1: Perform image closing. Simple dilation followed by erosion. 
    '''
    footprint = disk(4)
    if option == 0:
        # Option 0: dilation and skeletonization (thinnning). Works for simple shapes
        output = skeletonize(dilation(image, footprint)).astype(np.uint8)
    else:
        # Option 1: Image Closing. Works for all images but not too well for move commands
        output = erosion(dilation(image, footprint), footprint)
    return output

def get_move_data(image):
    '''
    Gets move data for every 20 data points after calling the DFS draw_lines function.
    '''
